...
describe("SubdivisionNFT", function () {
  ...
  describe("mintSubdivision", function () {
    ...
    it("Should revert if asset type isn't land(0) or estate(2)", async function () {
      ...
      //token id will be 3
      await deedNFT.connect(contractOwner).mintAsset(deedOwner.address, "0x", 1, "10 211 fake Addy");
      //token id will be 4
      await deedNFT.connect(contractOwner).mintAsset(deedOwner.address, "0x", 3, "10 211 fake Addy");
      //SubNFT minted with tokenID 1
      await expect(
