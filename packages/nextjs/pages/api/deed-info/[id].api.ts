...
import { downloadFile as downloadFileFromDatabase } from "~~/servers/databases/files.db";
...
const get = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const client = createPublicClient({
    chain: sepolia,
    transport: http(`${sepolia.rpcUrls.alchemy.http[0]}/${scaffoldConfig.alchemyApiKey}`),
  });
...
const downloadFile = async (file: IpfsFileModel, res: NextApiResponse) => {
  ...
  let stream: Readable;
  if (file.restricted) {
    stream = downloadFileFromDatabase(filename);
  } else {
    const filePath = `${process.env.NEXT_PINATA_GATEWAY}/ipfs/${filename}?pinataGatewayToken=${process.env.NEXT_PINATA_GATEWAY_KEY}`;
